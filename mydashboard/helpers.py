...
def saved_searches(employer, candidate):
	...
	employer_microsites = Microsite.objects.filter(company__in=employer_companies).values_list('url', flat=True)
	...
	return [url for url in candidate_urls if urlparse(url).netloc in employer_domains]
